# Topics I Want to Learn

A running list of subjects to explore. Pick from here when starting a learning session.

## Self-Reliance & Practical Skills
- [ ] Basic home repair and maintenance
- [ ] Growing food (container gardening, raised beds)
- [ ] Water purification methods
- [ ] Food preservation (canning, dehydrating, fermenting)
- [ ] Basic first aid and emergency preparedness
- [ ] Basic electrical and plumbing
- [ ] Vehicle maintenance fundamentals
- [ ] Cooking from scratch — real meals, not recipes

## Knowledge & Understanding
- [ ] Personal finance and investing basics
- [ ] How local and federal government actually works
- [ ] History — themes and patterns, not just dates
- [ ] Science literacy — how to read and evaluate studies
- [~] Philosophy — stoicism, practical ethics, decision-making *(in progress — see `knowledge/stoicism.md`)*
- [ ] Nutrition — what actually matters vs. marketing

## Technology & Tools
- [ ] Deepening my coding/automation skills
- [~] **Learning Claude deeply** — prompting, Claude Code, custom skills, API, building workflows *(ACTIVE — this is a core goal, not just a topic)*
- [~] **Building romi-01** — improving this repo's automation, scripts, prompts
- [ ] Privacy and digital security basics

## Health & Body
- [ ] Strength training fundamentals
- [ ] Mobility and flexibility
- [ ] Sleep optimization
- [ ] Stress management techniques

---
*Add topics as curiosity strikes. Check them off as you feel confident. A checked box means "I could explain this to someone else."*
